			...
			// 굳이 삼항연산자 준것은 null값을 처리하려고 넣었다. null과 그냥 ''으로 공백이 있는것은 다르다.(이게 nvl 처리하는것과 같은 역할을 한다.)
	                ...
	                if(jsonMsg.sessionId == $("#sessionId").val()){
	                	if(jsonMsg.msg == "" && jsonMsg.imgSrc != ""){
							$("#chatting_content").append("<div class ='d_img'><span class='msg_time'>"+time+"</span><img class='mePic' src='${pageContext.request.contextPath}/upload/"+jsonMsg.imgSrc+"'></div>");	
						...
						if(jsonMsg.msg == "" && jsonMsg.imgSrc != ""){
							$("#chatting_content").append("<div class ='d_img'><img class='otherPic' src='${pageContext.request.contextPath}/upload/"+jsonMsg.imgSrc+"'></div><span class='otmsg_time'>"+time+"</span>");	
							$("#chatting_content").scrollTop($("#chatting_content")[0].scrollHeight);
						}else{
							$("#chatting_content").append("<div class ='boxother'>" + jsonMsg.userName +"<br><span class='others'>" + jsonMsg.msg + "</span><span class='otmsg_time'>"+time+"</span>");
							$("#chatting_content").scrollTop($("#chatting_content")[0].scrollHeight);
						}
...
//		document.getElementById("modal").style.display="flex";
	...
	function fileUploadBtnClick() {
		if($("#fileUploadForm").css("display") == 'none'){
			$("#fileUploadForm").show();
		}else{
			$("#fileUploadForm").hide();
...
<body>
 <jsp:include page="/WEB-INF/views/base/header2.jsp" flush="true">
	...
	<button onclick="windowopen()">오픈채팅만들기</button><!-- location.href='openChatList' -->
		...
		<!-- 아작스에서 포이치로 돌려서 채팅대화내역리스트 뽑아서 여기에 어펜드 시키는것 -->
			<div class="chatting">
				...
				<form  id="fileForm" method="post" enctype="multipart/form-data">
						...
						<label class="fileUploadLabel" for="fileUpload">사진 등록</label>
						<input name="fileUpload" type="file" id="fileUpload" accept=".jpg,.jpeg,.gif,.png" onchange="imgAjax()">
				</form>		
				</div>
					<button id="testFile" onclick="fileUploadBtnClick()">파일 창 올라오기</button>
						...
						$(MsgList).each(
								function(){
									...
									if($("#userName").val() == send_user_id){
										...
										if(msg_pic != null){
											str += "<div class='d_img'><span class='msg_time'>"+msg_time+"</span><img src = '${pageContext.request.contextPath}/upload/"+msg_pic+"' class='mePic'></div>";
										...
										if(msg_pic != null){
											str += "<div class='d_img'></div><img src = '${pageContext.request.contextPath}/upload/"+msg_pic+"' class='otherPic' ><span class='otmsg_time'>"+msg_time+"</span>";
					...
					success: function (parti) {
						$(parti).each(
							function () {
								...
								else if(user_id == this.user_id) {
									$("#chatting_name").html("나 : "+this.user_id);
								}
