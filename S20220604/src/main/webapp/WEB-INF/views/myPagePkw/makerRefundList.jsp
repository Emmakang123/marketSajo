...
<!--	
	mypage	:	총결제번호(pay_tot_num), 수량(quantity), 취소처리상태(refund_status)
	payment :	구매자아이디(user_id), 총결제 금액(pay_tot), 결제일자(pay_date)
	product	:	상품판매번호(pro_num), 상품판매글제목(pro_title)
-->
	<section>
	<div id="myPage">
		<h3><i class="fa-solid fa-pipe" ></i>메이커 취소처리 리스트</h3>
		 	<table class="table table-hover" id="myPageTbl">
				...
				<c:choose>
					<c:when test="${listSize > 0}">
					  <c:forEach var="item" items="${listMakerRefund}">
						<tr>
							<td>
					...
					<c:otherwise>
						<tr>
							<td colspan="9">취소진행중인 상품이 없습니다.</td>
						</tr>
					</c:otherwise>
				</c:choose>
			</table>
