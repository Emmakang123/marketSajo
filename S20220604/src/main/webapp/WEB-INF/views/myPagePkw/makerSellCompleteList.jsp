...
<head>
<meta charset="UTF-8">
<title>makerSellCompleteList</title>
...
<!--	
	mypage	:	총결제번호(pay_tot_num), 수량(quantity), 주문처리상태(order_status), 구매확정상태(purchase_status)
	payment :	구매자아이디(user_id), 총결제 금액(pay_tot), 결제일자(pay_date)
	product	:	상품판매번호(pro_num), 상품판매글제목(pro_title)
-->
	<section>
	<div id="myPage">
 		<h3><i class="fa-solid fa-pipe" ></i>메이커 판매완료 리스트</h3>
		 	<table class="table table-hover" id="myPageTbl">
				...
				<c:choose>
					<c:when test="${listSize > 0}">
					  <c:forEach var="item" items="${listMakerSellCompleteList}">
						<tr>
					...
					<c:otherwise>
						<tr>
							<td colspan="9">판매진행 및 판매완료 내역이 없습니다.</td>
						</tr>
					</c:otherwise>
				</c:choose>
			</table>
