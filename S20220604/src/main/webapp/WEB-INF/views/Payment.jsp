...
<body>
	...
	<table>
		...
		<table id="pay_product">
			...
			<tr>
				...
				<td><input type="text" class="pro_title" value="${productInfo.pro_title}"></td>
				<td><input type="text" value="${productQuantity[status.index]}"></td>
				<td><input type="text" value="${productQuantity[status.index] * productInfo.pro_price}"></td>
			</tr>
			</c:forEach>  
		</table>			
	
	<input type="button" onclick="sendOrder()">
	...
	<table>
		...
		<tr>
			<td>총결제 금액: </td><td><input type="text" id="product_price" value="${total_price}"></td>
		</tr>
		<tr>
			<td>결제방법</td> 
    ...
    if(${basketNum} > 0) {
      function sendBasketData() {
    	...
    	var form = document.createElement('form');
  		...
  		form.setAttribute('action', "http://localhost:8989/AfterBuyUpdate");
    ...
    function sendUseCoupon() {
    	var user_id = document.getElementById('user_id').value;
    	var use_cupon = document.getElementById('selCp_num').value;
    	
    	var form = document.createElement('form');
		form.setAttribute('method','post');
		form.setAttribute('action', "http://localhost:8989/AfterBuyCouponUpdate");
		document.charset = "utf-8";
	...
	function checkButton() {    
		...
		IMP.request_pay({
			...
			if (rsp.success) {
				...
				if(document.getElementById('selCp_num').value > 0) 
					sendUseCoupon(); */
